...
import { PeriodType, useGetChart, usePostNewChart, usePutChart } from "@/api/charts";
...
interface ConfigureChartProps {
    chartId?: string;
}

export function ConfigureChart({ chartId }: ConfigureChartProps) {
        ...
        try {
            ...
            a.mutate({
                chart: {
                    ...
                    sources: createSource() as SourceResponse[],
                    segment: null,
                    position: data?.chart.position || 0,
                }
            })

            toast.success("Gráfico editado com sucesso!");
        } catch (error: any) {
    ...
    return (
        <div className="flex flex-col w-full gap-3 px-4">
            ...
            <div className="flex w-[360px] lg:w-full lg:gap-5 justify-between lg:mr-0 lg:justify-end">
                <Button
                    variant={"pointer"}
                    onClick={!chartId ? handleCreateChart : handleEditChart}
                    disabled={isPostNewChartPending}
                    className="w-40"
                >
                    {!chartId ? "Criar Gráfico" : "Salvar Alterações"}
                </Button>
            </div>
            <Toaster />
        </div>
